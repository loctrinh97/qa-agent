...
One factory file per module: `<MODULE>.locators.ts`.

Locator priority order: `getByRole` → `getByLabel` → `getByTestId` → `getByText` → CSS (last resort).

Generated by `/do-cucumber-task`. Do not edit manually — re-run the command to update.
```
...
- Set `BASE_URL` env var to your app's local URL for Playwright live locator healing
- Locator files live in `locators/<MODULE>.locators.ts` (factory function pattern)
- Page objects live in `pages/<CLASS>Page.ts`, extend `BasePage`
```
...
If the project is a web (Playwright) project, additionally grep the real
page-object files to detect what locator strategy the project actually uses
most:

```bash
grep -rn 'getByRole\|getByLabel\|getByTestId\|getByText\|locator(' . \
  --include='*.ts' --include='*.js' \
  --exclude-dir=node_modules --exclude-dir=dist 2>/dev/null | head -40
```

Record in `selectors-locators.md` under a `## Detected locator strategy`
heading:
```markdown
## Detected locator strategy
Primary: <highest-usage method>
Fallback order observed: <list in frequency order>
Note: <any method with 0 hits — e.g. "project rarely uses getByLabel">
```
"not determined — no page object files found" if nothing is evidenced.
If the project is not a web project (mobile/backend), omit this section.
